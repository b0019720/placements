...
<%@ page import="placements.Placement" %>
<!doctype html>
<html>
	<head>
			...
			<ol class="property-list placement">
				...
				<li class="fieldcontain">
					<span id="status-label" class="property-label"><g:message code="placement.status.label" default="Status" /></span>
						...
						<span class="property-value" aria-labelledby="status-label"><g:link controller="status" action="show" id="${placementInstance?.status?.id}">${placementInstance?.status?.encodeAsHTML()}</g:link></span>
